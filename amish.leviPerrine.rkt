...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname amish.leviPerrine) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(define listOfReservedWords '(if let while set!-env))
       ...
       (cond
         ...
         ((reservedWord? lcExp)
          (cond
            ...
            ((eq? lcExp 'set!-env) (list 'set!-env-exp))            
...
(define eval-if-exp
  (lambda (appExp env)
    (let ((boolExp (eval-exp (car appExp) env))
      ...
      (if boolExp (eval-exp trueExp env) (eval-exp falseExp env)))))
​
(define eval-while-exp
  (lambda (appExp env)
    (if (eval-exp (car appExp) env)
        (eval-while-exp (list (car appExp) (cadr appExp)) (extend-env-4-lambda (car (eval-exp (cadr appExp) env)) (cadr (eval-exp (cadr appExp) env)) env))
        'finished)))
​
(define eval-set!-exp
  (lambda (appExp env)
    (list (list (cadar appExp)) (list (eval-exp (cadr appExp) env)))))
    ...
    (cond
      ...
      (else
       (cond
         ...
         ((eq? (list-ref (list-ref lce 1) 0) 'set!-env-exp)
          (eval-set!-exp (cddr lce) env))
         (else
...
​
(define anExp '(let ((a 5)) (while (!= a 0) (set!-env a (- a 1)))))
